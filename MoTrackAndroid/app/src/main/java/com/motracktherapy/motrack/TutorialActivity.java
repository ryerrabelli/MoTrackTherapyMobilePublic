...
public class TutorialActivity extends AppCompatActivity {
    ...
    // c++ methods
    public native boolean inputImage(int width, int height, ByteBuffer srcBuffer, Surface previewSurface, boolean jTesting);//, ByteBuffer pixels, int w, int h);
    ...
    public native boolean getDoneState();
    ...
    private boolean jTesting = false; // true if testing, false is actually using/demoing
    private int useBackCamera = 1; // 1 for back camera, 0 for front camera
    ...
    public static int orientationGame = -5;
    private static int exerciseGameNum; // the exercise game ID
    ...
    int displayedWidth;
    int displayedHeight;
    TutorialActivity thisActivity = this;
    ...
    private ImageReader.OnImageAvailableListener mImageAvailable = new ImageReader.OnImageAvailableListener() {
        ...
        if (getDoneState()) {
            ...
            resultIntent.putExtra("CVACTIVITY_ERROR", "");
        ...
        if (lastImage != null && !paused) {
            ...
            boolean calibResult = inputImage(displayedWidth, displayedHeight, displayedBuffer, mPreviewSurface, jTesting);//, lemonBuffer, bMap.getHeight(), bMap.getWidth());
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int nrtbc_each = startingIntent.getIntExtra("NRTBC_EACH", -1);
        resultIntent.putExtra("NRTBC_EACH", nrtbc_each); //make it available again in the final screen
        int hand = startingIntent.getIntExtra("WHICH_HAND_INTENDED", -1);
        String json_data = startingIntent.getStringExtra("JSON_DATA");
        if (json_data == null) json_data = "";
        if (nrtbc_each < 0) {
            String error = "Reps of " + nrtbc_each + " is invalid";
            Toast.makeText(getApplicationContext(), error, Toast.LENGTH_LONG).show();
            resultIntent.putExtra("CVACTIVITY_GAMESCORE", 0);
            resultIntent.putExtra("CVACTIVITY_ERROR", error);
    ...
    private void setUpHandSizeRefImages(int exercise_game) throws IOException {
        ...
        } else if (exercise_game == FIST_PLAIN_GAME || exercise_game == FIST_SQUEEZE_BOTTLE_GAME || exercise_game == FIST_PUMP_BALLOON_GAME) { //squeeze bottle and pump balloon
    ...
    private void setUpAndroidUserInterface() {
        ...
        if (!jTesting) { // remove the testing buttons
            //reset button
    ...
    private static Size chooseOptimalSize(Size[] choices, int textureViewWidth,
        ...
        if (useMaxResolution) return new Size(maxWidth,maxHeight);
