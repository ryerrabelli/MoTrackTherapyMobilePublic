...
##-----------------------Build Net Fully connvolutional layers------------------------------------------------------------------------------------
        self.conv7 = tf.layers.separable_conv2d(inputs=self.conv6_1 , filters = 512,#self.conv6_1.get_shape()[3].value,
                                   kernel_size = 3, strides = 1, padding='same', activation = tf.nn.relu, name = "conv7")
        ...
        self.conv8 = tf.layers.separable_conv2d(inputs=self.conv7 , filters = 2,#self.conv6_1.get_shape()[3].value,
                                   kernel_size = 3, strides = 1, padding='same', activation = tf.nn.relu, name = "conv8")
