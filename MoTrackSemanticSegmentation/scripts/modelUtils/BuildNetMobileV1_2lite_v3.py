...
##-----------------------Build Net Fully connvolutional layers------------------------------------------------------------------------------------
        ...
        self.upsample_1 = tf.keras.layers.UpSampling2D(4)(self.conv6)
        self._debug(self.upsample_1)
        self.concat_t1 =  tf.keras.layers.Concatenate()([self.upsample_1,self.conv2_2])
        self._debug(self.concat_t1)
         
        self.upsample_2 = tf.keras.layers.UpSampling2D(2)(self.concat_t1)
        
        self.concat_t2 =  tf.keras.layers.Concatenate()([self.upsample_2,self.conv1_1_BN])
        self._debug(self.concat_t2)
        
        self.Prob = tf.layers.conv2d_transpose(inputs=self.upsample_2,
            filters=2,
            kernel_size=16,
            strides=(2, 2),
            padding='same')
