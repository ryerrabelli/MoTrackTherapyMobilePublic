...
#-----------------------------Build network encoder based on MobileNetV1 network and load the trained VGG16 weights-----------------------------------------
        ...
        self.convt_1 = tf.layers.conv2d_transpose(inputs=self.conv6_1,
            filters=64,
            kernel_size=64,
            strides=32,
            padding='same', activation = tf.nn.relu6)
        self.Prob = tf.contrib.layers.fully_connected(self.convt_1,2)
        '''
##-----------------------Build Net Fully connvolutional layers------------------------------------------------------------------------------------
        ...
        self.conv7 = tf.layers.conv2d_transpose(inputs=self.conv6_1,
            filters=512,
            kernel_size=7,
            strides=1,
            padding='same', activation = tf.nn.relu6)
        ...
        #W8= utils.weight_variable([1, 1, 512, NUM_CLASSES],name="W8")#4096], name="W8")  # 1X1 Convloution
        #b8 = utils.bias_variable([NUM_CLASSES], name="b8")
        #self.conv8 = utils.conv2d_basic(self.relu7, W8, b8)  # 1X1 Convloution
        #self._debug(self.conv8)
        self.conv8 = tf.layers.conv2d_transpose(inputs=self.conv7,
            filters=NUM_CLASSES,
            kernel_size=1,
            strides=1,
            padding='same')
        
        self.conv_t1 = tf.layers.conv2d_transpose(inputs=self.conv8,
