...
class BUILD_NET:
    ...
    def build(self, rgb, NUM_CLASSES, keep_prob,
        ...
        self.concat_t1 = tf.keras.layers.Concatenate()([self.conv_t1, self.conv3_2])
        ...
        self.concat_t2 = tf.keras.layers.Concatenate()([self.conv_t2, self.conv2_2])
        self._debug(self.concat_t2)

        self.conv_t3 = tf.layers.conv2d_transpose(inputs=self.concat_t2,
                                                  filters=8,
                                                  kernel_size=4,
                                                  strides=(2, 2),
                                                  padding='same')
        self._debug(self.conv_t3)
        self.concat_t3 = tf.keras.layers.Concatenate()([self.conv_t3, self.conv1_1_BN])
        self._debug(self.concat_t3)
