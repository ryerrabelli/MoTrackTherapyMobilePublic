...
class BUILD_NET:
    ...
    def build(self, rgbm, NUM_CLASSES, keep_prob,
        ...
        # Layer 1
        self.conv1_1 = tf.layers.conv2d(rgbm, 3, strides=1, padding='same', kernel_size = 3)  # Build Convolution layer and load weights
        self._debug(self.conv1_1)
        self.Prob = tf.layers.conv2d(inputs=self.conv1_1,
                                               filters=2,
                                               kernel_size=3,
                                               strides=1,
                                               padding='same')
