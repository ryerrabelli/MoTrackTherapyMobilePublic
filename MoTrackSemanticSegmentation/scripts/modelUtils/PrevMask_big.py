...
#-----------------------------Build network encoder based on MobileNetV1 network and load the trained VGG16 weights-----------------------------------------
        ...
        #Layer 1
        self.conv1_1 = tf.layers.batch_normalization(tf.layers.conv2d(rgb, filters=32,
            kernel_size=3, strides=2,
            padding='same', activation = tf.nn.relu6))
        self._debug(self.conv1_1)
        # Layer 2
        self.conv2_1 = tf.layers.conv2d(inputs=self.conv1_1, filters=128,
            kernel_size=3,
            padding='same', activation = tf.nn.relu6)
        self.conv2_2 =  tf.layers.conv2d(inputs=self.conv2_1, filters=128,
            kernel_size=3,strides =2,
            padding='same', activation = tf.nn.relu6)
        self._debug(self.conv2_2)
        # Layer 3
        self.conv3_1 = tf.layers.conv2d(inputs=self.conv2_2, filters=256,
            kernel_size=3,strides =2,
            padding='same', activation = tf.nn.relu6)
        self.conv3_2 = tf.layers.conv2d(inputs=self.conv3_1, filters=256,
            kernel_size=3,strides =1,
            padding='same', activation = tf.nn.relu6)
        self._debug(self.conv3_2)
        # Layer 4
        self.conv4_1 = tf.layers.conv2d(inputs=self.conv3_2, filters=512,
            kernel_size=3,strides =1,
            padding='same', activation = tf.nn.relu6)
        self.conv4_2 = tf.layers.conv2d(inputs=self.conv4_1, filters=256,
            kernel_size=3,strides =2,
            padding='same', activation = tf.nn.relu6)
        self._debug(self.conv4_2)
        # Layer 5
        self.conv5_1 = tf.layers.conv2d(inputs=self.conv4_2, filters=512,
            kernel_size=3,strides =1,
            padding='same', activation = tf.nn.relu6)
        self.conv5_2 = tf.layers.conv2d(inputs=self.conv5_1, filters=512,
            kernel_size=3,strides =1,
            padding='same', activation = tf.nn.relu6)
        self.conv5_3 = tf.layers.conv2d(inputs=self.conv5_2, filters=512,
            kernel_size=3,strides =1,
            padding='same', activation = tf.nn.relu6)
        self.conv5_4 = tf.layers.conv2d(inputs=self.conv5_3, filters=512,
            kernel_size=3,strides =1,
            padding='same', activation = tf.nn.relu6)
...
##-----------------------Build Net Fully connvolutional layers------------------------------------------------------------------------------------
        ...
        self.conv_t2 = tf.layers.conv2d_transpose(inputs=self.concat_t1,
            filters=16,
            kernel_size=4,
            strides=(2, 2),
            padding='same')
        self._debug(self.conv_t2)
        self.concat_t2 =  tf.keras.layers.Concatenate()([self.conv_t2,self.conv1_1])
        self._debug(self.concat_t2)
        
        self.Prob = tf.layers.conv2d_transpose(inputs=self.concat_t2,
            filters=2,
            kernel_size=16,
            strides=(2, 2),
            padding='same')
