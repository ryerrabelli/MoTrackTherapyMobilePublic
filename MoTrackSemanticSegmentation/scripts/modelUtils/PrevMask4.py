...
class BUILD_NET:
    ...
    def build(self, rgbm, NUM_CLASSES, keep_prob,
        ...
        self.conv2_1 = tf.layers.batch_normalization(tf.layers.conv2d(inputs = self.conv1_1, filters = 8, strides=1, padding='same', kernel_size = 3))
        self._debug(self.conv2_1)
        self.conv3_1 = tf.layers.batch_normalization(tf.layers.conv2d(inputs = self.conv2_1, filters = 8, strides=1, padding='same', kernel_size = 3))
        self._debug(self.conv3_1)
        self.Prob = tf.layers.conv2d(inputs=self.conv3_1,
                                               filters=2,
                                               kernel_size=3,
                                               strides=1,
                                               padding='same')
