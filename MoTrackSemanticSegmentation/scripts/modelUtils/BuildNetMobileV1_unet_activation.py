...
##-----------------------Build Net Fully connvolutional layers------------------------------------------------------------------------------------
        self.conv7 = tf.layers.batch_normalization(tf.layers.conv2d(self.conv5_4, 512,3, padding='same', use_bias = False, activation = tf.nn.relu6))
        self._debug(self.conv7)
        self.upsample_1 = tf.keras.layers.UpSampling2D(2)(self.conv7)
        self._debug(self.upsample_1)
        self.concat_1 = tf.keras.layers.Concatenate()([self.upsample_1,self.conv3_2])
        self._debug(self.concat_1)
        
        self.conv8 = tf.layers.batch_normalization(tf.layers.conv2d(self.concat_1, 256,3, padding='same', use_bias = False, activation = tf.nn.relu6))
        self._debug(self.conv8)
        self.upsample_2 = tf.keras.layers.UpSampling2D(2)(self.conv8)
        self._debug(self.upsample_2)
        self.concat_2 = tf.keras.layers.Concatenate()([self.upsample_2,self.conv2_2])
        self._debug(self.concat_2)
        
        self.conv9 = tf.layers.batch_normalization(tf.layers.conv2d(self.concat_2, 128,3, padding='same', use_bias = False, activation = tf.nn.relu6))
        self._debug(self.conv9)
        self.upsample_3 = tf.keras.layers.UpSampling2D(2)(self.conv9)
        self._debug(self.upsample_3)
        self.concat_3 = tf.keras.layers.Concatenate()([self.upsample_3,self.conv1_1])
        self._debug(self.concat_3)
        
        self.conv10 = tf.layers.batch_normalization(tf.layers.conv2d(self.concat_3, 64,3, padding='same', use_bias = False, activation = tf.nn.relu6))
        self._debug(self.conv10)
        self.upsample_4 = tf.keras.layers.UpSampling2D(2)(self.conv10)
        self.Prob = tf.layers.conv2d(self.upsample_4, 2,3, padding='same', use_bias = False, name = "PRob")
