...
################################Initiate folders were files are and list of train images############################################################################
    def __init__(self, ImageDir,GTLabelDir="", BatchSize=1, Suffle=True, partition = None, prevMask_SetFile = None):
        ...
        if partition is None:
            ...
            for each in os.listdir(self.Image_Dir):
                if each in partition:
                    if each.endswith('.PNG') or each.endswith('.JPG') or each.endswith('.TIF') or each.endswith('.GIF') or each.endswith('.png') or each.endswith('.jpg') or each.endswith('.tif') or each.endswith('.gif'):
                        self.OrderedFiles.append( each)
        if prevMask_SetFile is not None:
            self.prevMask_set = True
            newset = []
            with open(prevMask_SetFile,'r') as prevFile:
                for line in prevFile:
                    newset.append((line.split(",")))
                self.OrderedFiles = newset
        else:
            self.prevMask_set = False
...
###########################Read and augment next batch of images and labels#####################################################################################
    def ReadAndAugmentNextBatch(self, rows,cols, prevMask = None, cropLarge = False, addContourClass = -1):
...
#.............Read image and labels from files.........................................................
           if self.prevMask_set:
               Img = misc.imread(self.Image_Dir + "/" + self.SFiles[self.itr][0])
               LabelName=self.SFiles[self.itr][0][0:-4]+".png"# Assume Label name is same as image only with png ending
               PrevMask = misc.imread(self.Label_Dir + "/" + self.SFiles[self.itr][1].strip())
           else:
...
#............Set Batch image size according to first image in the batch...................................................
           if cropLarge:
              ...
              if len(ones[0]) > 0:
                  ...
                  Label = Label[min_x:max_x,min_y:max_y]
           if f==0:
               if rows ==-1 or cols ==-1:
                    Sy, Sx,d = Img.shape
                    Sy,Sx
                    ...
                    # if landscape, rows and cols would switch
                    if Img.shape[0] < Img.shape[1]:
                        ...
                        Sx = Crx = cols
               if self.prevMask_set:
                    Images = np.zeros([batch_size,Cry,Crx,4], dtype=np.float)
               else:
...
#..........Resize and strecth image and labels....................................................................
           ...
           if self.prevMask_set:
               PrevMask = misc.imresize(PrevMask, [Sy,Sx], interp='bilinear')
               PrevMask = PrevMask.reshape((PrevMask.shape[0],
                                            PrevMask.shape[1],
                                            1))
               Img = np.concatenate((Img,PrevMask),2)
           ...
           if not (Cry==Sy and Crx==Sx):
               for u in range(501):
                   ...
                   if np.sum(Label[Yi:Yi+Cry,Xi:Xi+Crx]>0)>MinOccupancy:
                      ...
                      break
           # add contour class (needs to be done before np.fliplr is called)
           if self.ReadLabels and addContourClass > 0:
                   im2, contours, hierarchy = cv2.findContours(Label,
                                    cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
                   # get biggest contour, assume it will be hand
                   if len(contours) > 0:
                       c = max(contours, key = cv2.contourArea)
                       Label = cv2.drawContours(Label, [c], 0, 2, addContourClass)
#------------------------Mirror Image-------------------------------# --------------------------------------------
...
######################################Read next batch of images and labels with no augmentation######################################################################################################
    def ReadNextBatchClean(self, rows, cols, cropLarge = False, addContourClass = -1 ): #Read image and labels without agumenting
        if self.itr>=self.NumFiles: # End of an epoch
            self.itr=0
            #self.SuffleBatch()
            self.Epoch+=1
...
##.............Read image and labels from files.........................................................
           if self.prevMask_set:
               Img = misc.imread(self.Image_Dir + "/" + self.OrderedFiles[self.itr][0])
               LabelName=self.OrderedFiles[self.itr][0][0:-4]+".png"# Assume Label name is same as image only with png ending
               PrevMask = misc.imread(self.Label_Dir + "/" + self.OrderedFiles[self.itr][1].strip())
           else:
...
#............Set Batch size according to first image...................................................
           if f==0:
                ...
                if self.prevMask_set:
                    Images = np.zeros([batch_size,Sy,Sx,4], dtype=np.float)
                else:
